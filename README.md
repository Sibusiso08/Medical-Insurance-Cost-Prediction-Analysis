...
The project covers the full analytics pipeline - raw data cleaning in Excel, structured querying in MySQL, interactive dashboards in Tableau and a **machine learning model** that predicts whether a patient is likely to be classified as high risk.
...
Three interactive dashboards were built in Tableau covering the full patient and claims story.

> To open the Tableau workbook, download the `.twb` file from the `tableau/` folder and connect it to `medical_insurance_cleaned.csv` when prompted.
...
Three columns were excluded from model features to prevent the model from using information it would not have access to in a real prediction scenario:
...
> The dataset (`medical_insurance_cleaned.csv`) must be in the `data/` folder. Update `CONFIG['data_path']` in Cell 1 of the notebook if your path differs.
